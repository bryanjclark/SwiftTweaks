...
private final class TweakDiskPersistency {
	...
	func loadFromDisk() -> TweakCache {
		...
		self.queue.sync {
			result = (try? Foundation.Data(contentsOf: self.fileURL))
				.flatMap {
					return try? NSKeyedUnarchiver.unarchivedObject(ofClass: Data.self, from: $0)
				}
				.map { $0.cache }
				?? [:]
		}
	...
	func saveToDisk(_ data: TweakCache) {
		self.queue.async {
			let nsData = try! NSKeyedArchiver.archivedData(withRootObject: Data(cache: data), requiringSecureCoding: false)
